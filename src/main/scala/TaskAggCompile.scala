...
object TaskAggCompile {
  ...
  object toState extends (TaskBehaviour ~> MapState) {
    ...
    override def apply[A](fa: TaskBehaviour[A]) = fa match {
      case Commit(id, text) => State.get.map(_ => Some(TaskAgg(id, text)))
      case Complete(t) => State.get.map(_ => t.map(_.copy(status = Completed)))
      case Reopen(t) => State.get.map(_ => t.map(_.copy(status = Open)))
